...
const Menu = ({ menuOpened, setMenuOpened, onSectionChange }: Props) => {
  ...
  return (
    <>
      ...
      >
        <div
          className={` h-1 rounded-xl w-full transition-all 
        ${
          menuOpened
            ? "rotate-45 translate-y-1 bg-blue-palette-600"
            : "bg-blue-palette-100"
        }`}
        />
        <div
          className={`bg-blue-palette-100 h-1 rounded-xl w-full my-1
        ${menuOpened && "hidden"}`}
        />
        <div
          className={` h-1 rounded-xl w-full transition-all 
      ...
      <div
        className={`z-10 fixed top-0 right-0 bottom-0 bg-blue-palette-100 transition-all overflow-hidden flex flex-col
        ${menuOpened ? "w-full md:w-[400px]" : "w-0"} `}
      >
...
const MenuItem = ({ label, onClick }: ItemProps) => {
  return (
    <button onClick={onClick} className="menu-label">
      {label}
    </button>
  );
};
